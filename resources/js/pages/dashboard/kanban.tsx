...
import { columnIds, columns, initialBoard } from './kanban/data';
import { KanbanColumn } from './kanban/kanban-column';
import { TaskCard } from './kanban/task-card';
import type { BoardState, ColumnId, Task } from './kanban/types';
import { findColumnId, findTask } from './kanban/utils';

interface KanbanProps {
    initialBoard?: BoardState;
}

export default function Kanban({ initialBoard: initialBoardProp }: KanbanProps) {
    const [board, setBoard] = React.useState<BoardState>(initialBoardProp ?? initialBoard);
    ...
    function handleDragOver(event: DragOverEvent) {
        ...
        setBoard((currentBoard) => {
            ...
            return {
                ...currentBoard,
                [activeColId]: activeItems.filter(
                    (task) => task.id !== activeId,
                ),
                [overColId]: [
    ...
    return (
        <div className="flex h-[calc(100dvh-var(--dashboard-header-height))] min-h-0 min-w-0 flex-col overflow-hidden">
            ...
            >
                <div className="min-h-0 min-w-0 flex-1 scrollbar-thin [scrollbar-color:var(--border)_transparent] overflow-x-auto overflow-y-hidden bg-muted/25 px-4 pt-4 pb-0 lg:px-5 lg:pt-5 [&::-webkit-scrollbar]:h-1 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-track]:bg-transparent">
